    ...
    for config_file in config_file_list:
        try:
            ...
            while True:
                ...
                obs, _, terminated, _, info = env.step(action) # obs: {"text": text_obs, "image": image_obs}
                state_info = {"observation": obs, "info": info} # save obs, image_obs should be som images
                image_array.append(obs['image'])
            ...
            screenshot = Image.fromarray(image_array[-1])
            screenshot_path = os.path.join(
                args.result_dir,
                "screenshots",
                f"screenshot_{task_id}.png",
            )
            if not os.path.exists(os.path.dirname(screenshot_path)):
                os.makedirs(os.path.dirname(screenshot_path), exist_ok=True)
            screenshot.save(screenshot_path)
            # screenshot.save('/home/ruis/code/visualwebarena/results/test_map_som9/screenshots/screenshots_0.png')
