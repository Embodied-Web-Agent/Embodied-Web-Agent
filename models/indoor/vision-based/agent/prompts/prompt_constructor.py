...
from browser_env.utils import StateInfo, pil_to_b64, pil_to_vertex, pil_to_byte_string
...
class MultimodalCoTPromptConstructor(CoTPromptConstructor):
    ...
    def get_lm_api_input(
        ...
        if "openai" in self.lm_config.provider or "gemini" in self.lm_config.provider:
            if self.lm_config.mode == "chat":
                message = [
                    {
                        "role": "system",
