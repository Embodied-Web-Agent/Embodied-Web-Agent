...
class RenderHelper(object):
    ...
    def render(
        ...
        # text observation
        observation = state_info["web_observation"]
        text_obs = observation["text"]
        info = state_info["web_info"]
        new_content = f"<h2>New Page</h2>\n"
        new_content += f"<h3 class='url'><a href={state_info['web_info']['page'].url}>URL: {state_info['web_info']['page'].url}</a></h3>\n"
