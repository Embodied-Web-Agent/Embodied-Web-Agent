...
def generate_from_gemini_chat_completion(
    ...
    for message in messages[:-1]:  # Exclude the last message (current turn)
        ...
        if role and parts:
            gemini_message = {"role": role, "parts": []}
            for part in parts:
                if part.get("text"):
                    gemini_message["parts"].append({"text": part["text"]})
                elif part.get("mime_type") and part.get("data"):
                    gemini_message["parts"].append({"mime_type": part["mime_type"], "data": part["data"]})
    ...
    if last_message.get("parts"):
        for part in last_message["parts"]:
            if part.get("text"):
                last_content_parts.append(part["text"])
            elif part.get("mime_type") and part.get("data"):
                last_content_parts.append({"mime_type": part["mime_type"], "data": part["data"]})


    response = chat.send_message(last_content_parts, generation_config=generation_config)
