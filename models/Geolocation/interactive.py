...
class Agent:
    ...
    def generate_web_query(self, image_messages, web_context):
        ...
        response = self.call_gpt(GENERATE_WEB_QUERY.format(query_history=query_history_str), context=image_messages)
    ...
    def evaluate_confidence(self, image_messages, web_context):
        ...
        if web_context:
            full_context.append({"type": "text", "text": "Here are some relevant web search results."})
            full_context.extend(web_context)

        response = self.call_gpt(ESTIMATE_CONFIDENCE, context=full_context)
        # print(response)
        confidence = self.parse(response, 'confidence')
        return confidence
    
    def generate_prediction(self, full_context):
        response = self.call_gpt(GENERATE_PREDICTION, context=full_context)
        prediction = parse_prediction(response)
        return response, prediction

    def call_gpt(self, prompt, max_tokens=400, temperature=0.7, context=None):
        if context:
...
def add_images(image_messages, coords, init_id):
    headings = [0, 90, 180, 270]
    directions = ["north", "east", "south", "west"]

    for deg, direction in zip(headings, directions):
        image, _ = get_street_view_from_api(GOOGLE_API_KEY, coords, heading=deg, size="600x400", cache_dir=f"interactive_views/{init_id}/{coords}")
...
def run(init_node, A, iter_num=5):
    ...
    add_images(image_messages, init_coords, init_node.id)
    ...
    for _ in range(iter_num):
        ...
        if valid_actions:
            ...
            if action.startswith("Move"):
                ...
                image_messages.append({"type": "text", "text": f"This is a standpoint adjacent to the initial. ID: {target_id}."})
                add_images(image_messages, coords, init_node.id)
                ...
                vwa_result = run_vwa(web_query)

                web_context.append({"type": "text", "text": f"{web_query['intent']}: {vwa_result}"})
...
def main():
    ...
    for _, row in sampled.iterrows():
        ...
        results_path = f"{args.output_dir}/{args.model}/{id}/result.json"
        ...
        output_dir = f"{args.output_dir}/{args.model}/{id}"
